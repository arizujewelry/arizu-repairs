...
// Sync old repair_counter into businesses table
try {
  const old = db.prepare('SELECT last_number FROM repair_counter WHERE id = 1').get();
  if (old) {
    db.prepare('UPDATE businesses SET repair_counter = ? WHERE id = 1 AND repair_counter <= 1000')
      .run(old.last_number);
  }
} catch(e) {}
...
function autoSeed() {
  ...
  const count = db.prepare('SELECT COUNT(*) as c FROM users WHERE business_id = 1 OR business_id IS NULL').get().c;
  if (count === 0) {
  ...
  if (!superExists) {
    ...
    console.log('✅ superadmin נוצר');
  }
}
try { autoSeed(); } catch(e) { console.error('Seed error:', e.message); }

// *** Flush WAL before accepting requests ***
